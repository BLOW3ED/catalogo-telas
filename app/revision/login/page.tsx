...
import { login } from "@/app/revision/actions";
import { SubmitButton } from "@/components/admin/SubmitButton";

export const metadata: Metadata = {
  title: "Entrar — Revisión de catálogo",
  robots: { index: false, follow: false },
};

export default async function RevisionLoginPage({
  searchParams,
}: {
  ...
  const { user } = await getSesionRevisor();
  if (user) redirect("/revision");
    ...
    <main className="mx-auto flex max-w-md flex-col px-4 py-16 sm:py-24">
      <div className="rounded-2xl border border-line bg-surface p-6 shadow-sm sm:p-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl text-ink-display">Revisión de catálogo</h1>
        ...
        <form action={login} className="flex flex-col gap-4">
          <label className="flex flex-col gap-1.5 text-sm font-medium text-ink-display">
            Correo
            <input
              type="email"
              name="email"
              required
              autoComplete="email"
              className="h-12 rounded-xl border border-line bg-surface px-4 text-base font-normal text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-heritage-navy"
            />
          </label>
          <label className="flex flex-col gap-1.5 text-sm font-medium text-ink-display">
            Contraseña
